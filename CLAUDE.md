...
- `src/lib/sound.ts` — rest-timer gong cue: a preloaded `<audio>` for `public/gong.mp3`, **unlocked on the set-complete tap** (mobile blocks audio until a gesture); `playRestDone()` fires at 0. **iOS ignores `HTMLAudioElement.volume`**, so loudness lives entirely in the asset (mastered loud — see `make-gong.mjs`); there is no JS volume lever
- `src/lib/keepalive.ts` — silent looping `<audio>` (`public/silence.wav`) started on the set-complete tap; keeps the page alive so the rest-timer gong fires when the screen is locked (**best-effort**). Stopped via `endRest()` in `logging.ts`; the `visibilitychange` catch-up listener self-removes when the logger DOM (`#lgEx`) is gone, so browser/gesture-back can't leak it. Hard limits (not solved): the iOS **mute switch** silences web audio, iOS has **no Vibration API**, and **low-power mode** can still suspend — the catch-up guarantees correct state on return even if the gong is missed.
- `scripts/make-gong.mjs` — synthesizes `public/gong.mp3` (license-clean additive synthesis; `node scripts/make-gong.mjs /tmp/gong.wav` then **master loud + encode** — a brickwall limiter (`alimiter`) raises perceived loudness ~6 dB to ~−12 LUFS with no EQ/timbre change, because iOS can't boost it at runtime; exact ffmpeg command in the file header)
