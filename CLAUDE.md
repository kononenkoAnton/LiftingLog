...
- `src/screens/logging.ts` — logging-mode screen (set table, timers, notes, finish/cancel). Rest timer is a **fixed bottom bar** (`.lg-rest` is `position:fixed`; `.lg.resting` reserves space) and plays a gong + vibrates at 0. The timer stays reliable when backgrounded via a **silent keep-alive loop** (`keepalive.ts`, started on the set-complete tap) plus a **`visibilitychange` catch-up** that fires the late cue from wall-clock on return (`fireRestDone`). All rest teardown goes through `endRest()` (stops timer + keep-alive); the catch-up listener self-removes when the logger DOM (`#lgEx`) is gone, so browser/gesture-back can't leak it. Hard platform limits (not solved): the iOS **mute switch** silences web audio, iOS has **no Vibration API**, and **low-power mode** can still suspend — the catch-up guarantees correct state on return even when the gong is missed.
...
- `src/lib/keepalive.ts` — silent looping `<audio>` (`public/silence.wav`) started on the set-complete tap; keeps the page alive so the rest-timer gong fires when the screen is locked (best-effort). Stopped via `endRest()` in `logging.ts`; a `visibilitychange` catch-up plus an `#lgEx`-presence self-heal keep it leak-free.
...
- `src/screens/history.ts` — past finished workouts (#/history). Exercise names link to the per-exercise history (`#/exercise/<encoded ref>`). Each expanded workout shows the **total weight lifted** + **per-exercise volume** (`workoutVolumeLb`/`exerciseVolumeLb`, full weight incl. bar, in the shared unit).
